...
=head2 Net::DNS::BIND::Manage

Provides dns zone management tools for Bind 9 similar to *h2n* (see "DNS
and BIND").
...
be under version control.  When the management tool is run it will
...
Use one of the following two methods for a normal  Perl 6 environment:
  ...
  panda install Net::DNS::BIND::Manage
